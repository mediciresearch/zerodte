...
// New function to generate LollipopData
...
export function generateLollipopData(numberOfPoints: number = 500): LollipopData[] {
    randomFactor = 25 + Math.random() * 25;
    const res = [];
    const date = new Date(Date.UTC(2018, 0, 1, 12, 0, 0, 0));
    for (let i = 0; i < numberOfPoints; ++i) {
        const time = (date.getTime() / 1000) as Time;
        const value1 = samplePoint(i);
        const value2 = -samplePoint(i + 1); // Generate negative values for value2
        res.push({
            time,
            value1,
            value2,
